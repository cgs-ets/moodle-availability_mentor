 ...
 * Condition main class.
 *
 * @package availability_mentor
 * @copyright  2022 Veronica Bermegui
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace availability_mentor;
...
class condition extends \core_availability\condition {
    ...
    public function get_description($full, $not, \core_availability\info $info) {
        ...
        if ($this->groupid) {
            ...
            if (!array_key_exists($this->groupid, self::$groupnames)) {
                $name = get_string('missing', 'availability_mentor');
            } else {
                $context = \context_course::instance($course->id);
                $name = format_string(
                    self::$groupnames[$this->groupid],
                    true,
    ...
    private function get_mentors_mentee($mentorid, $info) {
        global $DB;
        ...
        $mentees = $DB->get_records_sql($sql);

        // Get the students available to access.
        list($sql, $params) = $this->get_user_list_sql(false, $info, true);
        $result = $DB->get_fieldset_sql($sql, $params);
        sort($result);

        if (count($mentees) > 0) {
            $menteesids = array_column($mentees, 'id');

            foreach ($menteesids as $id) {
